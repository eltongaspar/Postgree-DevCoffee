...
SELECT 'PAGAMENTO_DIRETO_COM_FINALIDADE'::text AS tipo,
	SUM(p.payamt) AS Geral, -- Soma geral de todas as transações
    SUM(CASE WHEN p.isreceipt = 'Y' THEN p.payamt ELSE 0 END) AS Receita, -- Soma apenas dos valores positivos
    SUM(CASE WHEN p.isreceipt = 'N' THEN p.payamt ELSE 0 END) AS Despesas -- Soma apenas dos valores negativos
  --  sum(
       -- CASE p.isreceipt
  ...
  WHERE p.c_charge_id > 0::numeric 
  		...
  		AND p.datetrx >= to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text) AND p.datetrx <= to_timestamp('2024-11-30'::text, 'YYYY-MM-DD'::text) 
...
SELECT 'ALOCACAO_CONTRA_FINALIDADE'::text AS tipo,
    SUM(alp.amount) AS Geral, -- Soma geral de todas as transações
    SUM(CASE WHEN alp.amount > 0 THEN alp.amount ELSE 0 END) AS Receita, -- Soma apenas dos valores positivos
    SUM(CASE WHEN alp.amount < 0 THEN alp.amount ELSE 0 END) AS Despesas -- Soma apenas dos valores negativos
  ...
  WHERE alp.c_payment_id IS NOT NULL AND alp.c_invoice_id IS NULL AND alp.c_order_id IS NULL AND alc.c_allocationline_id IS NOT NULL 
  		...
  		AND h.dateacct >= to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text) AND h.dateacct <= to_timestamp('2024-11-30'::text, 'YYYY-MM-DD'::text) 
  		AND p.ad_client_id = 5000017::numeric
--  GROUP BY 'ALOCACAO_CONTRA_FINALIDADE'::text, p.ad_client_id, p.ad_org_id, p.c_bpartner_id, p.user1_id, p.user2_id, p.c_activity_id, p.c_charge_id, 
  ...
  SELECT 'ALOCACAO_FATURA'::text AS tipo,
    sum(il.linenetamt) * max(invoicepaidtodate.valor_alocado) /
        ...
        END AS valor_pagamento,
   -- Soma positiva de valor_pagamento
    sum(
        CASE 
            WHEN (il.linenetamt) * invoicepaidtodate.valor_alocado / 
                CASE
                    WHEN i.totallines = 0::numeric THEN 1::numeric
                    ELSE i.totallines
                END > 0 THEN
                (il.linenetamt) * invoicepaidtodate.valor_alocado / 
                CASE
                    WHEN i.totallines = 0::numeric THEN 1::numeric
                    ELSE i.totallines
                END
            ELSE 0
        END
    ) AS valor_pagamento_positivo,
    -- Soma negativa de valor_pagamento
    sum(
        CASE 
            WHEN (il.linenetamt) * invoicepaidtodate.valor_alocado / 
                CASE
   ...
   FROM c_payment p --pagamentos 
     ...
     LEFT JOIN LATERAL ( SELECT al.c_invoice_id,
          				...
          				AND ah.dateacct >= to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text) AND ah.dateacct <= to_timestamp('2024-11-30'::text, 'YYYY-MM-DD'::text)
 ...
 SELECT 'ANTECIPACAO'::text AS tipo,
    ...
    SUM(
        CASE 
            ...
            ELSE 0
        END 
        - COALESCE(paymnentopentodate.valor_aberto, 0::numeric)
    ) AS valor_pagamento_positivo, -- Soma de valores positivos
    SUM(
        CASE 
            WHEN p.isreceipt <> 'Y'::bpchar THEN p.payamt * '-1'::numeric
            ELSE 0
        END 
        - COALESCE(paymnentopentodate.valor_aberto, 0::numeric)
    ) AS valor_pagamento_negativo -- Soma de valores negativos
   FROM c_payment p --pagamentos
     LEFT JOIN cof_c_planofinanceiro pf ON pf.cof_c_planofinanceiro_id = p.cof_c_planofinanceiro_id --plano financeiro
     ...
     LEFT JOIN LATERAL ( SELECT al.c_payment_id,
          ...
          WHERE ah.isactive = 'Y'::bpchar AND (ah.docstatus = ANY (ARRAY['CO'::bpchar, 'CL'::bpchar])) 
          AND ah.dateacct between to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text) and to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text)
          GROUP BY al.c_payment_id) paymnentopentodate ON paymnentopentodate.c_payment_id = p.c_payment_id
  WHERE p.datetrx >= to_timestamp('2024-11-01'::text, 'YYYY-MM-DD'::text) AND p.datetrx <= to_timestamp('2024-11-30'::text, 'YYYY-MM-DD'::text) 
